...
onmessage = function (oEvent) {
  populateIndex = function (data) {
    ...
    this.entries = data.entries.map(this.createEntry).filter(function (n) {
      return n !== undefined;
    });

    this.entries.forEach(function (entry) {
      if (entry != null) this.lunr_index.add(entry);
    });
  ...
  search = function (data) {
    ...
    postMessage({
      query: data.query,
      results: results,
      type: { quicksearch: data.quicksearch },
    });
  };

  // if we're asked to index, index! else, search
  if (oEvent.data.type.index) populateIndex(oEvent.data);
  else search(oEvent.data);
};
